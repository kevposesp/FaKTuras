<template>
  <div class="view">
    <div
      class="bg-white rounded-lg shadow-lg px-8 max-w-xl mx-auto"
      :class="p ? 'py-5' : ''"
      ref="pdfSection"
    >
      ...
      <div class="grid grid-cols-4 w-full text-left mb-8 text-xs">
        ...
        <div
          v-for="(line, index) in invoice.invoices_lines"
          :key="index"
          class="border-b flex col-span-4"
        >
...
const exportPdf = async () => {
  ...
  await nextTick()
  exportToPDF(
    invoice.value.number_invoice + '-' + invoice.value.formatted_date + '.pdf',
    pdfSection.value,
    { orientation: 'p' },
    { html2canvas: { scale: 0.7, useCORS: true }, margin: [50, 0, 50, 15], autoPaging: 'text', width: 600, windowWidth: 600 }
  )
  generatePdf()
}
